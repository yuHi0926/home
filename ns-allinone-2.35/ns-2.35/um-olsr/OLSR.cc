...
OLSR::mpr_computation() {
	...
	//各インターフェースに対してMPR計算を行う必要がある
   
	state_.clear_mprset();
   //cout << endl;
  // cout << "nodeID [ "<< OLSR::node_id(ra_addr()) << " ] "<< endl;
   ...
   // Nはノードの隣接ノードのサブセットであり、これはインタフェースIの隣接ノードである。

	for (nbset_t::iterator it = nbset().begin(); it != nbset().end(); it++){
		if ((*it)->status() == OLSR_STATUS_SYM){ // I think that we need this check
			N.push_back(*it);                     // 私はこのチェックが必要だと思う
      }
   }

	// N2 is the set of 2-hop neighbors reachable from "the interface I",
   // N2は、「インタフェースI」から到達可能な2ホップの近隣の集合である。 
   // excluding:
   // 除外
	// (i)   the nodes only reachable by members of N with willingness WILL_NEVER
   // (i) NのwillingnessがWILL_NEVERのノード
	// (ii)  the node performing the computation
   // (ii)  計算を実行するノード //自分自身だと思われる
	// (iii) all the symmetric neighbors: the nodes for which there exists a symmetric
	//       link to this node on some interface.
   //       すべての対称隣接ノード：あるインタフェース上のこのノードへの対称リンクが存在するノード。

	for (nb2hopset_t::iterator it = nb2hopset().begin(); it != nb2hopset().end(); it++) {
		OLSR_nb2hop_tuple* nb2hop_tuple = *it;
		bool ok = true;
		OLSR_nb_tuple* nb_tuple = state_.find_sym_nb_tuple(nb2hop_tuple->nb_main_addr()); 
      //nb_tupleのnb_main_addr()とnb2hop_tupleのnb_main_addr()が同じであるかのチェック symmetric check
		if (nb_tuple == NULL) //NULLをreturnした場合
			ok = false;
		else { // tupleをreturnした場合
			nb_tuple = state_.find_nb_tuple(nb2hop_tuple->nb_main_addr(), OLSR_WILL_NEVER);
         //tupleのwillingnessの値がOLSR_WILL_NEVER(0)
			if (nb_tuple != NULL)
				ok = false;
			else {
				nb_tuple = state_.find_sym_nb_tuple(nb2hop_tuple->nb2hop_addr());
            //nb2hop_addr()を渡してみて、tupleをreturnした場合//本来は返さない
				if (nb_tuple != NULL)
					ok = false;
			}
		}

      //すべてのチェックを通り抜けたらN2に挿入する
		if (ok){
   ...
   // N_willingnessがWILL_ALWAYSに等しいNのすべてのメンバーからなるMPRセットから始めます。
	for (nbset_t::iterator it = N.begin(); it != N.end(); it++) {
		OLSR_nb_tuple* nb_tuple = *it;
		if (nb_tuple->willingness() == OLSR_WILL_ALWAYS){
      //nb_tupleにあるwillingness()がOLSR_WILL_ALWAYSの場合mprset_に挿入する
			state_.insert_mpr_addr(nb_tuple->nb_main_addr());
     }
	} 
	// 2. Calculate D(y), where y is a member of N, for all nodes in N.
	// We will do this later.
   // D（y）を計算する。ここでyはNのすべてのノードについてNのメンバーである。
   // これは後で行います。
   ...
   // MPRセットにNのノードを追加します。
   // これはN2のノードに到達可能性を提供する*唯一のノードです。
   // MPRセット内のノードでカバーされているN2からノードを削除します。
   // N2_tuple確認
   //cout << endl;
   //cout << "<N2_tuple>" << endl;
   int x = 0;
   for (nb2hopset_t::iterator it = N2.begin(); it != N2.end(); it++) {
     //cout << "2hopノード                   : " << (*it)->nb2hop_addr() << endl;
     //cout << "2hopノードへ繋がる1hopノード : " << (*it)->nb_main_addr() << endl;
     x++;
   }
   //cout << endl;
   //cout << "<N_tuple>" << endl;
   for (nbset_t::iterator it = N.begin(); it != N.end(); it++) {
     //cout << "1hopノード                   : " << (*it)->nb_main_addr() << endl;
   }
   //cout << endl;
   //
   //cout << "-----2hop先へ唯一繋がる1hopノードの選定-----" << endl;
   int i = 1;
	mprset_t foundset;
	std::set<nsaddr_t> deleted_addrs;
   for (nb2hopset_t::iterator it = N2.begin(); it != N2.end(); it++) { 
			//2hop先を見てる itはnb2hopset_t
		OLSR_nb2hop_tuple* nb2hop_tuple1 = *it;
      
      //cout << "######## Roop [" << i << "] ##########"  << endl;
      //cout << "確認する2hopノード ==> "  << nb2hop_tuple1->nb2hop_addr() << endl;
      i++;
      ...
      if (pos != foundset.end()){
		...
		for (nbset_t::iterator it2 = N.begin(); it2 != N.end(); it2++) {
			...
			//Nの1hopノードとN2の1hopノードの比較 it2はnbset_t
            //cout << "-----(1)Nの1hopノードとN2の1hopノードの比較-----" << endl;
	    //cout << "if ((*it2)->nb_main_addr() == nb2hop_tuple1->nb_main_addr())" << endl;
            //cout << "(*it2)->nb_main_addr()        : " << (*it2)->nb_main_addr() << endl;
            //cout << "nb2hop_tuple1->nb_main_addr() : " << nb2hop_tuple1->nb_main_addr() << " --> " << nb2hop_tuple1->nb2hop_addr() << endl;
            found = true;//同じならfoundをtrueにする
				break;
			}
		}
      //cout << endl;
		if (!found)//foundがfalseなら以下の処理をスキップ
			continue;
		
		found = false;
		for (nb2hopset_t::iterator it2 = it + 1 ; it2 != N2.end(); it2++) { // it + 1
            ...
            //cout << "-----(2)N2の2hopノードを比較-----" << endl;
	    //cout << "if (nb2hop_tuple1->nb2hop_addr() == nb2hop_tuple2->nb2hop_addr())" << endl;
            //cout << "nb2hop_tuple1->nb2hop_addr()  : " << nb2hop_tuple1->nb2hop_addr() << " <-- " << nb2hop_tuple1->nb_main_addr() << endl;
            //cout << "nb2hop_tuple2->nb2hop_addr()  : " << nb2hop_tuple2->nb2hop_addr() << " <-- " << nb2hop_tuple2->nb_main_addr() << endl;
				found = true;
				break;
			}
		}
      //cout << endl;
		...
		if (!found) {
         ...
         //cout << "MPRに[ " << nb2hop_tuple1->nb_main_addr() << " ]を選択" << endl;
	 //cout << endl;
			...
			for (nb2hopset_t::iterator it2 = it + 1; it2 != N2.end(); it2++) {
              ...
              //MPRに決定したノードにつながる2hop先のノードを消す
					deleted_addrs.insert(nb2hop_tuple2->nb2hop_addr());
	//cout << "MPRに選択されたノードに繋がる2hopノード[ " << nb2hop_tuple2->nb2hop_addr() << " ]を消す"<< endl;
					it2 = N2.erase(it2);
					it2--;
  				}
			}
		...
		for (std::set<nsaddr_t>::iterator it2 = deleted_addrs.begin();
			...
			for (nb2hopset_t::iterator it3 = N2.begin();
               ...
               // 後者の削除によって無効にされた可能性があるため、
               // 外部イテレータをリセットする必要があります
					it = N2.begin();
					it--;
				}
			}
   ...
   //cout << endl;
   //cout << "-----2hopノードを多くカバーできる1hopノードの選定-----" << endl;
	while (N2.begin() != N2.end()) {
		// 4.1. For each node in N, calculate the reachability, i.e., the
		// number of nodes in N2 which are not yet covered by at
		// least one node in the MPR set, and which are reachable
		// through this 1-hop neighbor
      // N内の各ノードについて、到達可能性、すなわちMPRセット内の少なくとも1つのノードによってまだカバーされておらず、この1ホップのネイバーを介して到達可能であるN2内のノードの数を計算する
		...
		for (nbset_t::iterator it = N.begin(); it != N.end(); it++) {
			...
			rs.insert(r); //2hop先のノードの数
			reachability[r].push_back(nb_tuple); // 2hop先のノードの数とその2hopにつながる1hopノード
		}
      ...
      // 到達可能性がゼロでないN個のノードの中で最も高いN_出現率を持つノードをMPRとして選択する。
      // 多項選択の場合、N2の最大ノード数に到達可能であるノードを選択する。
      // 複数のノードが同じ到達可能性を提供する場合は、D（y）が大きいMPRとしてノードを選択します。
      // MPRセット内のノードによって現在カバーされているN2からノードを削除する
		...
		if (max != NULL) {
         ...
         //cout << "MPRに[ " << max->nb_main_addr() << " ]を選択" << endl;
	 //cout << endl;
   ...
   //cout << "<<ノード " << OLSR::node_id(ra_addr()) <<" のMPR{" ;
...
OLSR::rtable_computation() {
	...
	// 2.新しいルーティングエントリが、宛先ノードとして対称隣接ノード（h = 1）から追加されます。
	for (nbset_t::iterator it = nbset().begin(); it != nbset().end(); it++) {
		OLSR_nb_tuple* nb_tuple = *it;
			...
			for (linkset_t::iterator it2 = linkset().begin(); it2 != linkset().end(); it2++) {
				...
				if (get_main_addr(link_tuple->nb_iface_addr()) == nb_tuple->nb_main_addr() && link_tuple->time() >= CURRENT_TIME) {
					...
					rtable_.add_entry(link_tuple->nb_iface_addr(),
							...
							1);
					//cout << "---------------------------------------------------------------"<< endl;
					//cout << "1hop先のノードへの経路" << endl;
					//cout << "dest    next    iface    dist" << endl;
					//cout << link_tuple->nb_iface_addr()<< "	" << link_tuple->nb_iface_addr() << "       "<< link_tuple->local_iface_addr() <<"        1"<< endl;
	...
	// (iii) all the symmetric neighbors: the nodes for which there exists a symmetric
	//       link to this node on some interface.
	// N2は、このノードから到達可能な2ホップのネイバの集合であり、以下を除く。
	//（i）Nのメンバーのみが到達可能なノードWILL_NEVER
	//（ii）計算を実行するノード
	//（iii）すべての対称的な近傍：対称的なものが存在するノード
	//いくつかのインタフェースでこのノードにリンクします。
	for (nb2hopset_t::iterator it = nb2hopset().begin(); it != nb2hopset().end(); it++) {
		OLSR_nb2hop_tuple* nb2hop_tuple = *it;
		bool ok = true;
		OLSR_nb_tuple* nb_tuple = state_.find_sym_nb_tuple(nb2hop_tuple->nb_main_addr());
		...
		// N2の各ノードに対して、ルーティングテーブルに新しいエントリを作成します
		if (ok) {
			OLSR_rt_entry* entry = rtable_.lookup(nb2hop_tuple->nb_main_addr());
			assert(entry != NULL);
			//if(!(entry->next_addr() ==4)){
			rtable_.add_entry(nb2hop_tuple->nb2hop_addr(),
					entry->next_addr(),
					entry->iface_addr(),
					2);
			//cout << "2hop先のノードへの経路" << endl;
			//cout << "dest    next    iface    dist" << endl;
			//cout << nb2hop_tuple->nb2hop_addr()<<"       " << entry->next_addr() << "       "<< entry->iface_addr() << "        2"<< endl;
			//}
		}
	}
	...
	for (u_int32_t h = 2; ; h++) {
		...
		// 4.1。 トポロジテーブル内の各トポロジエントリについて、
		// そのT_dest_addrがルーティングテーブル内のルートエントリのR_dest_addrに対応しておらず、
		// そのT_last_addrがR_distがhに等しいルートエントリのR_dest_addrに対応する場合、
		// 新しいルートエントリを記録しなければならない ルーティングテーブルに（存在しない場合）
		for (topologyset_t::iterator it = topologyset().begin();
			...
			//cout << "遠隔ノードへの経路" << endl;		
			//cout << "dest    next    iface    dist" << endl;
			//cout <<topology_tuple->dest_addr() <<"       "<< entry2->next_addr() <<"       "<< entry2->iface_addr() <<"        " <<entry2->dist()+1<< endl;
				added = true;
			}
		}
		...
		// 5.複数のインタフェース関連ベース内の各エントリに対して、次のようなルーティングエントリが存在する。
		// R_dest_addr == I_main_addr（複数インタフェース関連エントリの）
		//そして次のようなルーティングエントリはありません：
		// R_dest_addr == I_iface_addr
		//ルーティングテーブルにルートエントリが作成されます
		for (ifaceassocset_t::iterator it = ifaceassocset().begin();
			...
			if (entry1 != NULL && entry2 == NULL) {
				rtable_.add_entry(tuple->iface_addr(),
						...
						entry1->dist());
			//cout << "rtable(5)" << endl;		
			//cout << "tuple->iface_addr() " << "entry1->next_addr() " << "entry1->iface_addr()" << "entry1->dist()" << endl;
			//cout <<"["<< tuple->iface_addr() <<"] ["<< entry1->next_addr() <<"] ["<< entry1->iface_addr() <<"] ["<< entry1->dist() <<"]"<< endl;
				added = true;
			}
		}
...
OLSR::process_hello(OLSR_msg& msg, nsaddr_t receiver_iface, nsaddr_t sender_iface) {
	...
	double rt = CURRENT_TIME;
	//cout <<"----------------------------------------"<<endl;
	//cout << "recv_hello_time : "<<rt<<endl;
	//cout <<"send_hello["<<sender_iface<<"] -> recv_hello["<<receiver_iface<<"]"<<endl;
	//cout <<"----------------------------------------"<<endl;
...
OLSR::forward_default(Packet* p, OLSR_msg& msg, OLSR_dup_tuple* dup_tuple, nsaddr_t local_iface) {
	...
	//	cout <<"node["<<ra_addr()<<"]"<<"foward_default_JITTER->"<<JITTER << endl;
			retransmitted = true;
		}
	}
...
//
/// \brief（0、delay）の遅延で送信される％OLSRメッセージを受信します。
///
/// このバッファリングシステムは、いくつかの％OLSRメッセージをピギーバックするために使用されます。
/// 同じ％OLSRパケット。
///
/// \param msg送信する必要がある％OLSRメッセージ。
/// \param delay％OLSRメッセージがバッファされる最大遅延。
void
OLSR::enque_msg(OLSR_msg& msg, double delay) {
	...
	//if(msg.msg_type() == OLSR_TC_MSG){
	//cout << "tctime" << CURRENT_TIME << endl;
	//cout <<"nodeID["<< ra_addr() << "]tc_delay : " << delay << endl;
//}
	//cout << "hello_delay_time : " << CURRENT_TIME <<endl;
	//if(msg.msg_type() == OLSR_HELLO_MSG){
	//cout <<"nodeID["<< ra_addr() << "]hello_delay : " << delay << endl;
//}
...
OLSR::send_hello() {
	...
	//cout <<"----------------------------------------"<<endl;
	//cout << "send_hello_time : " << now << endl;
	//cout << "send_hello["<< ra_addr() <<"]"<< endl;
	//cout <<"----------------------------------------"<<endl;
	...
	for (linkset_t::iterator it = linkset().begin(); it != linkset().end(); it++) {
		...
		if (link_tuple->local_iface_addr() == ra_addr() && link_tuple->time() >= now) {
			...
			msg.hello().hello_msg(count).nb_iface_addr(i) =
				link_tuple->nb_iface_addr();
			//cout << "recv_hello["<< link_tuple->nb_iface_addr() <<"]"<< endl;
			//cout <<"----------------------------------------"<<endl;
...
OLSR::send_tc() {
	...
	enque_msg(msg, JITTER);
	//cout <<"node["<<ra_addr()<<"]"<<"send_tc_JITTER->"<<JITTER << endl;
}
...
OLSR::add_topology_tuple(OLSR_topology_tuple* tuple) {
	debug("%f: Node %d adds topology tuple: dest_addr = %d last_addr = %d seq = %d\n",
		...
		tuple->seq());
	//	cout << "topology_tuple[ra:dest:last]" << ra_addr() << ":" << tuple->dest_addr() << ":" << tuple->last_addr() << endl;
