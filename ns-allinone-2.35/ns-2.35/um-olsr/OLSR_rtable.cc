...
///データパケットをネクストホップに転送するために使用する必要のある適切なエントリを検索します（宛先が指定されている場合）。
///
...
///次のようなルーティングテーブルを想像してみてください：[A、B] [B、C] [C、C]; [dest addr、next-hop addr]という形式の各ペアです。 
///この場合、この関数が[A、B]で呼び出された場合、CはA宛てのデータパケットを転送するために使用されなければならない次のホップであるため、[C、C]のペアが返されます。 
///このノードの、しかしBはそうではありません。 この関数は、パケットを転送するための適切なネイバーを見つけます。
///
...
OLSR_rtable::find_send_entry(OLSR_rt_entry* entry) {
	...
	while (e != NULL && e->dest_addr() != e->next_addr())
		...
		std::cout <<"rtable_serch"<<std::endl;
		std::cout <<"entry->dest["<<entry->dest_addr()<<"]"<<std::endl;
		std::cout <<"entry->next["<<entry->next_addr()<<"]"<<std::endl;
		std::cout <<"entry->iface["<<entry->iface_addr()<<"]"<<std::endl;
		std::cout <<"-----------------------------"<<std::endl;
		std::cout <<"e->dest["<<e->dest_addr()<<"]"<<std::endl;
		std::cout <<"e->next["<<e->next_addr()<<"]"<<std::endl;
	return e;
}
